<template>
  <div class="ingredients-container max-w-full mx-20">
    ...
    <div class="ingredient-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-8 overflow-auto">
      <div v-for="(ingredient, index) in itemsArray" :key="index" class="rounded ingredient-box relative flex items-center justify-center border border-white p-4 mb-4"
        ...
        <img v-show="showDelete[index]" @click="deleteIngredient(index)" src="/delete-icon.png" alt="Delete Icon" class="delete-icon w-3 h-3 mt-2 mr-2 absolute top-0 right-0 cursor-pointer">
      </div>
    </div>
    <div class="add-ingredient flex items-center mt-8">
      <input v-model="newIngredient" type="text" placeholder="Add Ingredient" class="rounded-md ingredient-input px-2 py-2 border border-white rounded-none text-white bg-transparent outfit-outfit flex-grow mr-4 w-full">
    ...
    <div class="justify-content text-center">
      <button @click="emitIngredients" class="rounded create-recipe-button border border-white text-white py-2 px-4 mt-4 text-lg flex items-center space-x-2 transition-transform">
        <span>Create Recipe</span>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
        </svg>
      </button>
      <button @click="emitFindRecipe" class="rounded find-recipe-button border border-white text-white py-2 px-4 mt-4 text-lg flex items-center space-x-2 transition-transform">
